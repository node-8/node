...
// A malformed prefix retains byte offsets for later alternatives. Replacement
// classes also match malformed subparts without requiring an end anchor.
assertMatchIndices(
...
for (const tail of ['', 'after']) {
  assertEquals([malformedSubject, malformedField, eAcute],
      Array.from(assertMatchIndices(
          [[0, 8], [4, 7], [5, 7]],
          expression('((' + malformedClass + '){2})'), malformedSubject + tail)));
}
