...
Fragment GetDefinitionImpl(const std::vector<char>& code,
  ...
  const char* resource_type = type == CodeType::kUtf8
                                  ? "StaticExternalUtf8Resource"
  ...
  constexpr size_t unit = 4;  // 0-255 and a ","
  ...
  int cur = snprintf(
      result.data(), def_size, literal_def_template, "uint8_t", var.c_str());

  assert(cur != 0);

#ifdef NODE_JS2C_USE_STRING_LITERALS
  memcpy(result.data() + cur,
         ascii_string_literal_start.data(),
         ascii_string_literal_start.size());
  cur += ascii_string_literal_start.size();
  memcpy(result.data() + cur, code.data(), code.size());
  cur += code.size();
  memcpy(result.data() + cur,
         utf_string_literal_end.data(),
         utf_string_literal_end.size());
  cur += utf_string_literal_end.size();
#else
  memcpy(result.data() + cur,
  ...
  for (size_t i = 0; i < count; ++i) {
    std::string_view str = GetCode(ptr[i]);
...
Fragment GetDefinition(const std::string& var, const std::vector<char>& code) {
  ...
  if (is_ascii) {
    ...
    return GetDefinitionImpl(code, var, CodeType::kAscii);
  }
  Debug("UTF-8, static size %zu\n", code.size());
  return GetDefinitionImpl(code, var, CodeType::kUtf8);
}
