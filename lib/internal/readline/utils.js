...
function charLengthLeft(str, i) {
  ...
  if (getOptionValue('--experimental-node-8-string-semantics')) {
    if (!NumberIsInteger(i) || i > str.length) return 0;
    if (StringPrototypeCharCodeAt(str, i - 1) < 0x80) return 1;
    let start = i - 1;
    // Look back at most three continuation bytes, never beyond the cursor.
    while (start > 0 && i - start < 4 &&
           (StringPrototypeCharCodeAt(str, start) & 0xc0) === 0x80) {
      start--;
    }
    const span = i - start;
    return getUtf8CharacterLength(str, start, span) === span ? span : 1;
  }
  if ((i > 1 &&
...
function charLengthAt(str, i) {
  ...
  if (getOptionValue('--experimental-node-8-string-semantics')) {
    if (!NumberIsInteger(i) || i < 0) return 1;
    if (StringPrototypeCharCodeAt(str, i) < 0x80) return 1;
    return getUtf8CharacterLength(str, i, 4);
  }
