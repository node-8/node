...
function emitKeypressEvents(stream, iface = { __proto__: null }) {
  ...
  if (getOptionValue('--experimental-node-8-string-semantics')) {
    ...
    // Terminal keys are Unicode, unlike node-8's raw byte streams.
    terminalDecoder = new TextDecoder('utf-8', { ignoreBOM: true });
  }
  ...
  function onData(input) {
    if (stream.listenerCount('keypress') > 0) {
      const string = terminalDecoder && ArrayBufferIsView(input) ?
        terminalDecoder.decode(input, { stream: true }) :
        stream[KEYPRESS_DECODER].write(input);
      if (string) {
