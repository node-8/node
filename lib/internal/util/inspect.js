...
const byteEscapeSequencesRegExp = /[\x00-\x1f\x27\x5c\x7f-\x9f\ud800-\udfff]/u;
const byteEscapeSequencesReplacer = /[\x00-\x1f\x27\x5c\x7f-\x9f\ud800-\udfff]/gu;
const byteEscapeSequencesRegExpSingle = /[\x00-\x1f\x5c\x7f-\x9f\ud800-\udfff]/u;
const byteEscapeSequencesReplacerSingle = /[\x00-\x1f\x5c\x7f-\x9f\ud800-\udfff]/gu;
...
function useByteStringEscapes() {
  try {
    return getOptionValue('--experimental-node-8-string-semantics');
  } catch (error) {
    // Inspect also formats bootstrap errors, before options are available.
    // Do not inspect this error or cache a snapshot-time semantics value.
    if (error?.code !== 'ERR_OPTIONS_BEFORE_BOOTSTRAPPING') throw error;
    return StringFromCodePoint(233).length === 2;
  }
}

function byteEscapeFn(str) {
  const first = StringPrototypeCharCodeAt(str);
  if (first < 128) return meta[first];
  const point = StringPrototypeCodePointAt(str, 0);
  // In byte mode \x80 means a raw byte, not the C1 scalar U+0080.
  return point < 160 ? `\\u00${StringPrototypeSlice(meta[point], 2)}` :
    `\\u${NumberPrototypeToString(point, 16)}`;
}

function strEscapeBytes(str, singleQuote) {
  let result = '';
  let last = 0;
  for (let i = 0; i < str.length; i++) {
    ...
    let escaped;
    let width = 1;
    if (point === singleQuote || point === 92 || point < 32 || point === 127) {
      escaped = meta[point];
    } else if (point === 0xc2) {
      const next = StringPrototypeCharCodeAt(str, i + 1);
      if (next >= 0x80 && next <= 0x9f) {
        escaped = `\\u00${StringPrototypeSlice(meta[next], 2)}`;
        width = 2;
      }
    } else if (point === 0xed) {
      const second = StringPrototypeCharCodeAt(str, i + 1);
      const third = StringPrototypeCharCodeAt(str, i + 2);
      if (second >= 0xa0 && second <= 0xbf && third >= 0x80 && third <= 0xbf) {
        const codePoint = 0xd000 | ((second & 0x3f) << 6) | (third & 0x3f);
        escaped = `\\u${NumberPrototypeToString(codePoint, 16)}`;
        width = 3;
      }
    }
    // All other bytes, including malformed sequences, remain unchanged.
    if (escaped !== undefined) {
      result += `${StringPrototypeSlice(str, last, i)}${escaped}`;
      i += width - 1;
      last = i + 1;
    }
  }
  return addQuotes(result + StringPrototypeSlice(str, last), singleQuote);
}

// Escape control characters, single quotes and the backslash.
// This is similar to JSON stringify escaping.
function strEscape(str) {
  const byteMode = useByteStringEscapes();
  let escapeTest = byteMode ? byteEscapeSequencesRegExp : strEscapeSequencesRegExp;
  let escapeReplace = byteMode ? byteEscapeSequencesReplacer : strEscapeSequencesReplacer;
  let singleQuote = 39;
  ...
  if (StringPrototypeIncludes(str, "'")) {
    ...
    if (singleQuote !== 39) {
      escapeTest = byteMode ? byteEscapeSequencesRegExpSingle : strEscapeSequencesRegExpSingle;
      escapeReplace = byteMode ? byteEscapeSequencesReplacerSingle : strEscapeSequencesReplacerSingle;
    }
  }
  ...
  if (str.length > 100) {
    str = RegExpPrototypeSymbolReplace(escapeReplace, str, byteMode ? byteEscapeFn : escapeFn);
    return addQuotes(str, singleQuote);
  }
  if (byteMode) return strEscapeBytes(str, singleQuote);
...
function formatProperty(ctx, value, recurseTimes, key, type, desc,
  ...
  if (typeof key === 'symbol') {
    ...
    const tmp = RegExpPrototypeSymbolReplace(
      byteMode ? byteEscapeSequencesReplacer : strEscapeSequencesReplacer,
      SymbolPrototypeToString(key),
      byteMode ? byteEscapeFn : escapeFn,
    );
