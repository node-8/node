...
const { namespace: { addDeserializeCallback, isBuildingSnapshot } } = require('internal/v8/startup_snapshot');
...
function createMatcher(pattern, options = kEmptyObject) {
  ...
  if (getOptionValue('--experimental-node-8-string-semantics')) {
    opts.node8QmarksTest = node8QmarksTest;
  }
  return new (lazyMinimatch().Minimatch)(pattern, opts);
}

function node8QmarksTest({ 0: pattern, 1: suffix = '' }, { dot, nocase }) {
  const count = pattern.length - suffix.length;
  if (nocase) suffix = StringPrototypeToLowerCase(suffix);
  return (file) => {
    if (file.length < count ||
        (dot ? file === '.' || file === '..' : StringPrototypeCharCodeAt(file, 0) === 46)) {
      return false;
    }
    if (!nocase && (file.length < count + suffix.length || !StringPrototypeEndsWith(file, suffix))) {
      return false;
    }
    let offset = 0;
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Decode the local character without clipping at the literal suffix.
      if (StringPrototypeCharCodeAt(file, offset) < 0x80) {
        offset++;
      } else {
        asciiPrefix = false;
        offset += getUtf8CharacterLength(file, offset, 4);
      }
    }
    if (!nocase || suffix.length === 0) return offset === file.length - suffix.length;
    const folded = StringPrototypeToLowerCase(file);
    if (!StringPrototypeEndsWith(folded, suffix)) return false;
    // Root-locale contextual lowercase (Final Sigma) preserves UTF-8 width.
    const prefixLength = asciiPrefix ? offset :
      StringPrototypeToLowerCase(StringPrototypeSlice(file, 0, offset)).length;
    return folded.length - prefixLength === suffix.length;
  };
}
...
if (isBuildingSnapshot()) {
  addDeserializeCallback(() => patternFnCache.clear());
}
