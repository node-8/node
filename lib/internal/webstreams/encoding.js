  ...
  constructor() {
    ...
    this.#transform = new TransformStream({
      transform: (chunk, controller) => {
        ...
        chunk = String(chunk);
        let finalChunk = '';
        for (const item of new SafeStringIterator(chunk)) {
          const codePoint = StringPrototypeCodePointAt(item, 0);
          if (this.#pendingHighSurrogate !== null) {
            const highSurrogate = this.#pendingHighSurrogate;
            this.#pendingHighSurrogate = null;
            if (0xDC00 <= codePoint && codePoint <= 0xDFFF) {
              finalChunk += StringFromCodePoint(
                ((highSurrogate - 0xD800) << 10) +
                codePoint - 0xDC00 + 0x10000);
              continue;
            }
            finalChunk += StringFromCodePoint(0xFFFD);
          }
