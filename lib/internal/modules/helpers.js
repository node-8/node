...
/**
 * Remove one initial BOM, represented as U+FEFF in stock strings or
 * EF BB BF in node-8 strings. Preserve every remaining unit unchanged.
 * @param {string} content
 * @returns {string}
 */
function stripBOM(content) {
  const first = StringPrototypeCharCodeAt(content);
  if (first === 0xFEFF) {
    content = StringPrototypeSlice(content, 1);
  } else if (first === 0xEF && StringPrototypeCodePointAt(content, 0) === 0xFEFF) {
    content = StringPrototypeSlice(content, 3);
  }
  return content;
}
