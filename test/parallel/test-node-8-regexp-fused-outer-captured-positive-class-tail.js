...
assert.strictEqual(
  /([A-C\u00e9-\u00eb]{2,5})x/du.exec(eAcute + 'x'), null);

const bounded = /([A-C\u00e9-\u00eb]{2,5})x/du.exec(
  eAcute + eCircumflex + 'x');
assert.notStrictEqual(bounded, null);
assert.deepStrictEqual(bytes(bounded[0]), bytes(eAcute + eCircumflex + 'x'));
assert.deepStrictEqual(bytes(bounded[1]), bytes(eAcute + eCircumflex));
assert.deepStrictEqual(Array.from(bounded.indices), [[0, 5], [0, 4]]);

const backtrack = /([x\u00e9-\u00eb]{2,5})x/du.exec(
  eAcute + eCircumflex + 'x');
assert.notStrictEqual(backtrack, null);
assert.deepStrictEqual(bytes(backtrack[1]), bytes(eAcute + eCircumflex));
assert.deepStrictEqual(Array.from(backtrack.indices), [[0, 5], [0, 4]]);

const nested = /((?<run>[A-C\u00e9-\u00eb]{1,3}))x/du.exec(
  eAcute + eCircumflex + 'x');
assert.notStrictEqual(nested, null);
for (let capture = 1; capture <= 2; capture++) {
  assert.deepStrictEqual(bytes(nested[capture]), bytes(eAcute + eCircumflex));
  assert.deepStrictEqual(nested.indices[capture], [0, 4]);
}
...
assert.deepStrictEqual(
  Array.from(malformedMatch.indices), [[1, 6], [1, 5]]);
assert.strictEqual(malformed.lastIndex, 6);

const all = Array.from((eAcute + eCircumflex + 'xAAx').matchAll(
  /([A-C\u00e9-\u00eb]{1,3})x/dgu));
assert.deepStrictEqual(
  all.map((match) => match.indices[0]), [[0, 5], [5, 8]]);
assert.deepStrictEqual(
  all.map((match) => match.indices[1]), [[0, 4], [5, 7]]);
