...
const bodyOnly = /([A-C\u00e9-\u00eb]){1,3}xy/du.exec(
...
const upperBound = /p=(([A-C\u00e9-\u00eb]){2,8})z/du.exec(
  'p=A' + eAcute + 'BC' + eCircumflex + 'ABCz');
...
// Exact, complete variable, and pure-outer sibling paths remain unchanged.
assert.deepStrictEqual(
  Array.from(/(([A-C\u00e9-\u00eb]){2})xy/du.exec(
    eAcute + eCircumflex + 'xy').indices),
  [[0, 6], [0, 4], [2, 4]]);
assert.deepStrictEqual(
  Array.from(/(([A-C\u00e9-\u00eb]){1,3})/du.exec(
